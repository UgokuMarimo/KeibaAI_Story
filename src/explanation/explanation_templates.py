...
# --- 特徴量名の日本語マッピング ---
# システム内部の変数名を、ユーザーに分かりやすい日本語に変換するための辞書
FEATURE_NAME_MAPPING = {
    ...
    "過去5走_着順_最小": "近走の最高着順",
    "過去5走_着順_最大": "近走の最低着順",
    "過去5走_上がり_平均": "近走の平均上がりタイム",
    "過去5走_上がり_最小": "近走の最速上がりタイム",
    "過去5走_上がり_最大": "近走の最も遅い上がりタイム",
    "過去5走_通過順_平均_平均": "近走の平均ポジショニング",
    "過去5走_通過順_変動_平均": "近走の脚質の自在性",
    "過去5走_賞金_平均": "近走の平均獲得賞金",
    "過去5走_賞金_合計": "近走の獲得賞金合計",
    "獲得賞金_合計": "通算獲得賞金",

    # 過去走統計 (相対評価 - レース内偏差値など)
    "過去5走_着順_平均_race_dev": "近走着順のメンバー内優位性",
    "過去5走_上がり_平均_race_dev": "近走の末脚のメンバー内優位性",
    "過去5走_通過順_平均_平均_race_dev": "先行力のメンバー内優位性",
    "過去5走_賞金_平均_race_dev": "近走実績のメンバー内優位性",
    ...
    "騎手複勝率": "騎手の通算複勝率",
    "騎手競馬場勝率": "騎手の当該コース勝率",
    "騎手年間騎乗数": "騎手の経験値（年間騎乗数）",
    
    # 騎手データ (相対評価)
    "騎手勝率_race_dev": "騎手勝率のメンバー内優位性",
    "騎手競馬場勝率_race_dev": "コース相性のメンバー内優位性",
    "jockey_id": "騎手ID",
    ...
    "馬番": "枠順（馬番）",
    "枠番": "枠番",
    "開催バイアス_馬番グループ": "枠順の有利不利（バイアス）",
    
    # 個別レース (Recent 1-5)
    "着順1": "前走着順", "着順2": "2走前着順", "着順3": "3走前着順", "着順4": "4走前着順", "着順5": "5走前着順",
    "人気1": "前走人気", "人気2": "2走前人気",
    "オッズ1": "前走オッズ", "オッズ2": "2走前オッズ",
    "上がり1": "前走上がり3F", "上がり2": "2走前上がり3F",
    "通過順_平均1": "前走の位置取り", "通過順_平均2": "2走前の位置取り",
    "距離差1": "前走との距離変化", "距離差2": "2走前との距離変化",
...
def get_feature_name_display(feature_name: str) -> str:
    """
    特徴量名を日本語表記に変換する。辞書にない場合は元の名前を返す。
    部分一致などのロジックも追加可能。
    """
    # 1. 完全一致チェック
    ...
    # 2. 部分一致チェック (例: _scaled などを除去してチェック)
    clean_name = feature_name.replace("_scaled", "").replace("_std", "").replace("_min", "最小").replace("_max", "最大").replace("_mean", "平均")
    if clean_name in FEATURE_NAME_MAPPING:
        return FEATURE_NAME_MAPPING[clean_name]

    # 3. 特定のパターン置換
    if "_race_dev" in feature_name:
        base_name = feature_name.replace("_race_dev", "")
        base_disp = get_feature_name_display(base_name)
        return f"{base_disp}（相対評価）"
...
EXPLANATION_TEMPLATES = {
    "default_positive": lambda feature, value_display, shap_value: f"「{get_feature_name_display(feature)}」（値: {value_display}）がポジティブな影響 ({shap_value:+.3f}) を与えています。",
    "default_negative": lambda feature, value_display, shap_value: f"「{get_feature_name_display(feature)}」（値: {value_display}）がネガティブな影響 ({shap_value:+.3f}) を与えています。"
}