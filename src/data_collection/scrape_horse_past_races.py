...
REQUEST_WAIT_TIME = 1.0 # サーバー負荷軽減のための待機時間
...
def scrape_all_past_races_from_horse_page(
    ...
    try:
        # 性別: div.horse_title -> .txt_01 (例: "牡5　鹿毛")
        horse_title = soup.find("div", class_="horse_title")
        if horse_title:
            txt_01 = horse_title.find("p", class_="txt_01")
            if txt_01:
                text = safe_get_text(txt_01) 
                if "牡" in text: sex_val = "牡"
                elif "牝" in text: sex_val = "牝"
                elif "セ" in text: sex_val = "セ"

        # 生年: table.db_prof_table -> 生年月日
        ...
        if prof_table:
            ...
            for row in rows:
                ...
                td = row.find("td")
                if th and td:
                    header = safe_get_text(th)
                    value = safe_get_text(td)
                    if "生年月日" in header:
                        # 2018年3月15日 -> 2018
        ...
        print(f"[SCRAPER WARN] Failed to scrape profile for {horse_id}: {e}")
    ...
    for row_idx, row in enumerate(rows):
        ...
        try:
            ...
            race_id_match = re.search(r'/race/(\d+)', race_link['href']) if race_link else None
            race_id = race_id_match.group(1) if race_id_match else ''
            
            # 頭数 (6)
            num_horses = safe_get_text(cols[6])
            # 枠番 (7)
            wakuban = safe_get_text(cols[7])
            # 馬番 (8)
            ...
            # 'm' がない場合もあるので、数字列を抽出するように変更
            distance_num_match = re.search(r'(\d+)', distance_full_str)
            distance = distance_num_match.group(1) if distance_num_match else ''

            # コース詳細 (内/外) の抽出
            # 例: '芝1600' -> '', '芝1600外' -> '外'
            # カッコで囲まれていたり、直接ついていたりするケースがある
            course_detail_match = re.search(r'(外|内|左|右|直|火|主)', distance_full_str)
            course_detail = course_detail_match.group(1) if course_detail_match else ''
            ...
            # 賞金 (28)
            # ページによってはカラム数が異なる場合があるため、後ろから数えるか、headerを確認するのが確実だが、
            # 現状は固定インデックスでトライし、失敗したらNone
            if len(cols) > 28:
                 prize_money_str = safe_get_text(cols[28])
                 prize_money = to_numeric_or_nan(prize_money_str)
            else:
                 prize_money = pd.NA
            ...
            if pd.notna(birth_year) and pd.notna(parsed_date):
                try:
                    race_year = parsed_date.year
                    age = race_year - birth_year
                except:
                    pass
            elif pd.notna(birth_year) and date_str:
                 # date_str (YYYY年MM月DD日) から年を抽出
                 match_year = re.search(r'(\d{4})年', str(date_str))
                 if match_year:
                     race_year = int(match_year.group(1))
                     age = race_year - birth_year
            ...
            record = {
                ...
                '体重': to_numeric_or_nan(weight),
                '体重変化': to_numeric_or_nan(weight_dif),
                '着差': to_numeric_or_nan(margin),
                'ペース': pace, # stringのまま
                '賞金': prize_money,
                '厩舎': pd.NA, # このページからは取得できないためNA
                '性': sex,
                '齢': age,
                'horse_id': horse_id, # IDを追加 (必須)
                'is_jra_race': is_jra,
